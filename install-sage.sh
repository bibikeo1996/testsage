...
# Lấy đường dẫn hiện tại (tương thích với Windows Git Bash)
CURRENT_DIR=$(pwd)

# Kiểm tra xem Sage theme đã tồn tại chưa
if [ -d "sage-sport/wp-content/themes/sage" ]; then
    ...
    docker run --rm -v "${CURRENT_DIR}/sage-sport:/var/www/html" -w "/var/www/html/wp-content/themes/sage" composer:latest composer install
    
    # Cài đặt Node.js dependencies
    echo "📦 Cài đặt Node.js dependencies..."
    docker run --rm -v "${CURRENT_DIR}/sage-sport:/var/www/html" -w "/var/www/html/wp-content/themes/sage" node:18 npm install
    ...
    docker run --rm -v "${CURRENT_DIR}/sage-sport:/var/www/html" -w "/var/www/html" composer:latest composer create-project roots/sage wp-content/themes/sage
